...
class _AgentSessionsPageState extends State<AgentSessionsPage> {
  ...
  void _fetchSessions() {
    ...
    if (!widget.githubAppOauth && widget.gitProvider != GitProvider.GITHUB) {
      if (mounted) setState(() => _loading = false);
      return;
    }
    ...
    manager.getCopilotAgentSessions(widget.accessToken, owner, repo).then((sessions) {
      if (!mounted || generation != _fetchGeneration) return;
      setState(() {
        _sessions.addAll(sessions);
        _loading = false;
  ...
  bool get _isGithubWithOauth =>
      widget.gitProvider == GitProvider.GITHUB &&
      (widget.githubAppOauth || widget.accessToken.isNotEmpty);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _isGithubWithOauth
          ? FloatingActionButton(
              backgroundColor: colours.tertiaryInfo,
              foregroundColor: colours.primaryLight,
              onPressed: _openCreateSession,
              child: const FaIcon(FontAwesomeIcons.plus),
            )
          : null,
